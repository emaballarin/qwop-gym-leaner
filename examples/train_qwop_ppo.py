"""
Training script for QWOP using StableBaselines3 PPO algorithm.

This script demonstrates how to train a PPO agent on the QWOP environment
using hyperparameters adapted from the original qwop-gym project.

PPO often works better than DQN for control tasks and is more sample-efficient.

Recommended training duration: 10-20M timesteps for competitive results.
...
eval_callback = EvalCallback(
    ...
    log_path=log_dir,
    eval_freq=25_000,  # Evaluate every 25k steps (more frequent than DQN)
    render=False,
    n_eval_episodes=10,
    deterministic=True,
    verbose=1,
...
# PPO hyperparameters (adapted from original qwop-gym)
model = PPO(
    "MlpPolicy",  # Use MLP policy for numeric observations
    env,
    verbose=1,
    device="cpu",  # Use CPU for MLP policies (more efficient than GPU)
    # Core PPO parameters
    learning_rate=1e-3,
...
print("=" * 80)
print("QWOP PPO Training Configuration")
print("=" * 80)
...
# Train the model
# Original qwop-gym recommends 10-20M timesteps for competitive results
...
model.save(os.path.join(log_dir, "ppo_qwop_final"))
print(f"✓ Final model saved to {log_dir}/ppo_qwop_final.zip")
...
env_test = VecVideoRecorder(
    ...
    name_prefix="best_model_qwop_ppo",
)
...
print("=" * 80)
print(f"✓ Final model: {log_dir}/ppo_qwop_final.zip")
